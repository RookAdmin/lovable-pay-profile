...
const PaymDetail = () => {
  ...
  const { data: paym, isLoading, refetch } = useQuery({
    queryKey: ["paym", id],
    queryFn: async () => {
          ...
          <Tabs defaultValue="details">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-2xl font-semibold">{paym.currency}{paym.amount}</p>
                    </div>
                    ...
                    <div>
                      ...
                      <div>
                        {paym.isPaid ? (
                          ...
                          <Badge variant="outline" className="bg-yellow-50 text-yellow-600 border-yellow-200">Expired</Badge>
                        ) : (
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-muted-foreground mb-1">Payment Link</h3>
                    ...
                    {paym.expiresAt && (
                      <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-1">Expiration</h3>
                        <div className="flex items-center">
                          <Calendar className="h-4 w-4 mr-2" />
                          <p>{format(paym.expiresAt, "MMM d, yyyy h:mm a")}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {paym.lastReminderSent && (
                      <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-1">Last Reminder</h3>
                        <div className="flex items-center">
                          <Send className="h-4 w-4 mr-2" />
                          <p>{format(paym.lastReminderSent, "MMM d, yyyy h:mm a")}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {paym.invoiceApp && (
                      <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-1">Invoice App</h3>
                    ...
                    {paym.invoiceId && (
                      <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-1">Invoice ID</h3>
                        <p>{paym.invoiceId}</p>
                      </div>
                    )}
                  </div>
                ...
                <CardFooter className="flex justify-between">
                  <Button
                    ...
                    className={paym.isPaid ? "" : "bg-green-500 hover:bg-green-600"}
                  >
                    {paym.isPaid ? (
                      <>
            ...
            <TabsContent value="reminders">
              <Card>
                ...
                <CardContent>
                  {reminders && reminders.length > 0 ? (
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-2 text-xs text-muted-foreground">
                              <div className="flex items-center">
                                ...
                                Scheduled: {format(reminder.scheduledAt, "MMM d, yyyy h:mm a")}
                              </div>
                              
                              {reminder.sentAt && (
                                <div className="flex items-center mt-1">
                                  <Send className="h-3 w-3 mr-1" />
                                  Sent: {format(reminder.sentAt, "MMM d, yyyy h:mm a")}
                                </div>
                              )}
                            </div>
                          </div>
                    ...
                    <div className="text-center py-12">
                      ...
                      {!paym.isPaid && !isExpired(paym) && (
                        <Button onClick={sendReminder} className="bg-dc2e3e hover:bg-dc2e3e/90">
                          <Send className="mr-2 h-4 w-4" /> Send Reminder
                        </Button>
                      )}
                    </div>
                ...
                <CardFooter>
                  {!paym.isPaid && !isExpired(paym) && reminders && reminders.length > 0 && (
                    <Button onClick={sendReminder} className="bg-dc2e3e hover:bg-dc2e3e/90">
                      <Send className="mr-2 h-4 w-4" /> Send Another Reminder
                    </Button>
                  )}
                </CardFooter>
              </Card>
                ...
                <CardContent className="space-y-8">
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Detailed analytics for individual payment links will be available soon
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            <CardContent>
              <div className="border rounded-lg p-4 bg-gray-50">
                ...
                <div className="text-center mb-6">
                  ...
                  <p className="text-3xl font-bold">{paym.currency}{paym.amount}</p>
                </div>
                ...
                {paym.expiresAt && (
                  <div className="flex items-center justify-center text-sm text-muted-foreground mb-6">
                    <Calendar className="h-4 w-4 mr-2" />
                    {isExpired(paym) ? (
                      <span>Expired on {format(paym.expiresAt, "MMM d, yyyy")}</span>
                    ) : (
