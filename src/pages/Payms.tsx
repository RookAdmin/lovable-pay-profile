...
const Payms = () => {
  ...
  const { data: payms, isLoading, refetch } = useQuery({
    queryKey: ["payms"],
    queryFn: async () => {
      ...
      return data.map(paym => ({
        ...
        lastReminderSent: paym.last_reminder_sent ? new Date(paym.last_reminder_sent) : undefined,
  ...
  const filteredPayms = payms?.filter(paym => 
      ...
      {isLoading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredPayms.map((paym) => (
            <Card key={paym.id} className={`shadow-sm hover:shadow-md transition-shadow ${paym.isPaid ? 'border-green-200' : isExpired(paym) ? 'border-yellow-200' : 'border-gray-200'}`}>
              <CardHeader className="pb-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-1">
                    ...
                    {paym.invoiceApp && (
                      <Badge variant="outline" className="bg-purple-50 text-purple-600 border-purple-200 capitalize">
                        {paym.invoiceApp}
                      </Badge>
                    )}
                  </div>
              ...
              <CardContent className="pb-2">
                <div className="flex justify-between items-center mb-4">
                  <div>
                    ...
                    {paym.expiresAt && (
                      <div className="flex items-center text-xs text-muted-foreground mt-1">
                        ...
                        {isExpired(paym) ? 
                          <span>Expired {formatDistance(paym.expiresAt, new Date(), { addSuffix: true })}</span> : 
                          <span>Expires {formatDistance(paym.expiresAt, new Date(), { addSuffix: true })}</span>
                        }
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center space-x-1">
                    <Button size="sm" variant="ghost" onClick={() => copyPaymLink(paym)} title="Copy link">
                      <Copy className="h-4 w-4" />
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => copyPaymMessage(paym)} title="Copy message">
                      <ExternalLink className="h-4 w-4" />
                    </Button>
                    {!paym.isPaid && !isExpired(paym) && (
                      <Button size="sm" variant="ghost" onClick={() => sendReminder(paym)} title="Send reminder">
                ...
                <div className="text-sm">
                  <p>
                    ...
                    <span className="font-mono text-xs">/payms/{paym.uniqueLink}</span>
                  </p>
                  
                  {paym.invoiceId && (
                    <p className="mt-1">
                      <span className="text-muted-foreground">Invoice ID: </span>
                      <span className="font-mono text-xs">{paym.invoiceId}</span>
                    </p>
                  )}
                  ...
                  {paym.lastReminderSent && (
                    <p className="mt-1 text-xs text-muted-foreground">
                      ...
                      Last reminder: {format(paym.lastReminderSent, "MMM d, h:mm a")}
                    </p>
                  )}
                </div>
              </CardContent>
