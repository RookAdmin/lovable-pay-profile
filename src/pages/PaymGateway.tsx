...
const PaymGateway = () => {
    ...
    const fetchPaym = async () => {
      try {
        ...
        const paymObj: Paym = {
          ...
          expiresAt: paymData.expires_at
            ? new Date(paymData.expires_at)
            : undefined,
          ...
          lastReminderSent: paymData.last_reminder_sent
            ? new Date(paymData.last_reminder_sent)
            : undefined,
          metadata: paymData.metadata
            ? (paymData.metadata as Record<string, any>)
            : undefined,
        };
  ...
  return (
    <>
      <Helmet>
        <title>Paym Gateway | Paym.me</title>
        <meta
          name="description"
          content="Pay securely using Paym.me payment gateway. Fast, private, and easy payments."
        />
      </Helmet>
      <div className="container px-4 py-8 mx-auto max-w-lg">
          ...
          <CardContent className="space-y-6">
            <div className="flex justify-center">
              {paym.isPaid ? (
                ...
                <Badge
                  variant="outline"
                  className="bg-yellow-50 text-yellow-600 border-yellow-200 px-3 py-1 text-base"
                >
                  Expired
                </Badge>
              ) : (
                <Badge
                  variant="outline"
                  className="bg-blue-50 text-blue-600 border-blue-200 px-3 py-1 text-base"
                >
                  Payment Pending
                </Badge>
              )}
              ...
              <div className="flex items-center justify-center text-sm text-muted-foreground">
                ...
                {isExpired(paym) ? (
                  <span>
                    ...
                    {format(paym.expiresAt, "MMM d, yyyy")}
                  </span>
                ) : (
                  <span>
                ...
                {isExpired(paym) ? (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center w-full">
                    ...
                    <p className="text-sm text-yellow-600">
                      Please contact {profile?.display_name || "the sender"} for
                      an updated payment link.
                    </p>
                  </div>
                ) : (
                  <Button
